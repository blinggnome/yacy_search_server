...
Poison Pill rules are managed separately on the same page. They use the same plain-text matching behavior, but a match treats the host as compromised or broadly unwanted: the crawler rejects the page, adds the host to `url.poison_pill.black`, removes indexed records for that host, and records the removed-record count in Recent Crawler Rule Actions. Use Poison Pills for host-wide indicators such as domain hijack text or other signatures where keeping any content from that host is undesirable.
