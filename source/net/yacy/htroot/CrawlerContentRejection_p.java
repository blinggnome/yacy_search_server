...
public class CrawlerContentRejection_p {
    ...
    public static serverObjects respond(@SuppressWarnings("unused") final RequestHeader header, final serverObjects post, final serverSwitch env) {
        ...
        for (int i = 0; i < rules.size(); i++) {
            ...
            prop.putNum("rules_" + i + "_count", i);
            prop.putHTML("rules_" + i + "_rule", rule);
            prop.put("rules_" + i + "_dark", (i & 1) == 0 ? "1" : "0");
            rulesText.append(rule).append('\n');
        }
        prop.put("rules", rules.size());
        prop.putHTML("rulesText", rulesText.toString());
